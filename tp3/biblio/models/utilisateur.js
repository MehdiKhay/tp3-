"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
exports.utilisateur = void 0;
var personne_js_1 = require("./personne.js");
var utilisateur = /** @class */ (function (_super) {
    __extends(utilisateur, _super);
    function utilisateur(id, nom, role) {
        var _this = _super.call(this, id, nom) || this;
        _this.role = role;
        return _this;
    }
    utilisateur.prototype.getrole = function () {
        return this.role;
    };
    utilisateur.prototype.peutemprunter = function () {
        return this.role === 'user' || this.role === 'admin';
    };
    utilisateur.prototype.peutgerer = function () {
        return this.role === 'admin';
    };
    return utilisateur;
}(personne_js_1.personne));
exports.utilisateur = utilisateur;
